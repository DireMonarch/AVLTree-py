...
class AVLTreeInOrderIterator(AVLTreeIterator.AVLTreeIterator):
    '''
    ...
    def _move_next(self):
        ...
        if self._status == AVLTreeIterator.StatusEnum.BEFORE_FIRST:
            ...
            self._stack.clear()
            return False
        elif self._current._right is not None:
            self._stack.append(self._current._right)
            self._stack_left_to_null()
    ...
    def _stack_left_to_null(self):
        '''Helper function that adds all left nodes, from the node on the top of the stack, onto the stack'''
